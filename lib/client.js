...
.dsh-bishu-runerr{margin-top:6px;font-size:11px;line-height:1.45;color:var(--dsw-alias-state-error-primary,#ef4444);background:color-mix(in srgb,var(--dsw-alias-state-error-primary,#ef4444) 8%,transparent);border:1px solid color-mix(in srgb,var(--dsw-alias-state-error-primary,#ef4444) 28%,transparent);border-radius:6px;padding:6px 8px;white-space:pre-wrap;word-break:break-all;cursor:default}
...
.dsh-bishu-entry-sub{}
.dsh-bishu-fallback{margin-top:10px;border-top:1px dashed var(--dsw-alias-border-l1,#2a2a2a);padding-top:8px}
...
`;
		...
		// ── BookTab (default) ───────────────────────────────────────
		function BookTab({ workspace, setWorkspace, pushError, dismissError, toast, gotoWorkflow, gotoRun, dirtyCheckRef }) {
			...
			// 角色补档表单（character-add 工作流）：workspace 由书籍 tab 当前选择自动带出
			const [charAddNames, setCharAddNames] = useState("");
			const [charAddModel, setCharAddModel] = useState("");
			const [charAddStarting, setCharAddStarting] = useState(false);
			...
			// 启动 character-add 工作流：与 WorkflowsTab.startRun 同一条 post("/run") + gotoRun 链路；
			// workflow_id = character-add，仅 character_names 必填，workspace / model 走当前书籍 tab 的选择。
			const submitCharacterAdd = () => {
				const names = String(charAddNames || "").trim();
				if (!workspace) { toast({ kind: "warn", text: "请先选择书籍工作区" }); return; }
				if (!names) { toast({ kind: "warn", text: "请填写角色名字（逗号分隔）" }); return; }
				setCharAddStarting(true);
				const sel = charAddModel;
				const model = sel ? { provider: sel.split("/")[0], model: sel.slice(sel.indexOf("/") + 1) } : undefined;
				post("/run", {
					workflow_id: "character-add",
					workspace: workspace,
					parameters: { character_names: names },
					model: model,
					node_models: {},
					approval: false,
				}).then((b) => {
					if (b && b.error) { toast({ kind: "error", text: b.error }); return; }
					toast({ kind: "ok", text: "已启动角色补档: " + names });
					setCharAddNames("");
					if (gotoRun) gotoRun({
						run_id: b.run_id,
						workflow_id: "character-add",
					...
					.finally(() => setCharAddStarting(false));
			};
			...
			return h("div", null,
				...
				// 角色补档：选中书籍后追加补建新角色深度档案（character-add 工作流）。
				// 与 WorkflowsTab.startRun 同一条 post("/run") + gotoRun 链路；workspace 走当前书籍 tab 的选择。
				h(SectionCard, { icon: h("svg", { viewBox: "0 0 16 16", width: "14", height: "14", fill: "none", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", strokeLinejoin: "round" }, h("path", { d: "M8 2.5v3M8 10.5v3M2.5 8h3M10.5 8h3" }), h("circle", { cx: 8, cy: 8, r: 2.5 })), title: "角色补档", sub: "为后期登场的新角色补建深度档案" },
					!workspace ? h("div", { className: "dsh-bishu-empty" }, "请先选择书籍工作区") :
					h("div", null,
						h("div", { style: { fontSize: 11, color: "var(--dsw-alias-label-secondary,#888)", marginBottom: 10, padding: "8px 10px", background: "var(--dsw-alias-bg-layer-2,#1c1c1c)", borderRadius: 8, border: "1px solid var(--dsw-alias-border-l1,#2a2a2a)" } },
							"为后期登场的新角色补建深度档案（信念+声线），追加合并、不影响已有档案"),
						h("div", { className: "dsh-bishu-field" },
							h("label", { className: "dsh-bishu-label" }, "角色名字", h("span", { className: "req" }, "*"), h("span", { className: "vkey" }, "character_names")),
							h("input", {
								className: "dsh-bishu-input",
								placeholder: "宇智波佐助, 旗木卡卡西",
								value: charAddNames,
								onChange: (ev) => setCharAddNames(ev.target.value),
							}),
							h("div", { className: "dsh-bishu-desc" }, "多个角色用半角逗号分隔；运行后追加合并到现有角色档案"),
						),
						h("div", { className: "dsh-bishu-field" },
							h("label", { className: "dsh-bishu-label" }, "运行模型（可选）"),
							h("select", {
								className: "dsh-bishu-input",
								style: { appearance: "auto" },
								value: charAddModel,
								onChange: (ev) => setCharAddModel(ev.target.value),
							},
								h("option", { value: "" }, "默认（主会话模型）"),
								providers.map((p) => p.models.map((m) => h("option", { key: p.id + "/" + m.id, value: p.id + "/" + m.id }, p.name + " · " + m.name))),
							),
							h("div", { className: "dsh-bishu-desc" }, "为该次补档指定专用模型；缺省继承主会话默认模型"),
						),
						h("div", { style: { marginBottom: 8, fontSize: 11, color: "var(--dsw-alias-label-secondary,#888)" } },
							"工作区：", h("span", { className: "dsh-bishu-mono", style: { fontFamily: "var(--dsw-font-mono,ui-monospace,Menlo,monospace)", color: "var(--dsw-alias-label-primary,#bbb)" } }, workspace)),
						h("button", {
							className: "dsh-bishu-btn dsh-bishu-btn-primary",
							disabled: charAddStarting || !String(charAddNames || "").trim(),
							onClick: submitCharacterAdd,
						}, charAddStarting ? "启动中…" : "▶ 启动角色补档"),
					),
				),
					...
					h(SectionCard, { icon: h("svg", { viewBox: "0 0 16 16", width: "14", height: "14", fill: "none", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", strokeLinejoin: "round" }, h("path", { d: "M2.5 3h11v10h-11z" }), h("path", { d: "M2.5 6.5h11" })), title: "运行记录", sub: "点行查看详情" },
						...
						filtered.map((r) => h("div", {
							...
							h("div", { className: "main" },
								...
								// 失败行在 meta 下方追加 error 摘要：截断到 80 字符，title 放全文（复用红字样式）
								(r.status === "failed" && r.error) ? h("div", {
									className: "dsh-bishu-runerr",
									title: String(r.error),
								}, String(r.error).slice(0, 80) + (String(r.error).length > 80 ? "…" : "")) : null,
		...
		function App({ controller }) {
			...
			return h("div", { className: "dsh-bishu-drawer" + (controller.getSnapshot().panelOpen ? "" : " dsh-bishu-hidden") },
				...
				h("div", { className: "dsh-bishu-body" },
					...
					tab === "book" ? h(BookTab, { workspace, setWorkspace, pushError, dismissError, toast: showToast, gotoWorkflow, gotoRun, dirtyCheckRef: bookDirtyCheckRef }) :
