...
export const apply = (ctx) => {
	...
	function safeJoinWorkspace(workspace, rel) {
		...
		const abs = join(w, r);
		// path.join 在 Windows 上使用反斜杠，但下面的边界检查只用正斜杠前缀。
		// 比较前先把 abs 规范化成正斜杠，避免每次合法保存都被误判「离开工作区」。
		const absFwd = String(abs).replace(/\\/g, "/");
		if (!absFwd.toLowerCase().startsWith(w.toLowerCase() + "/")) {
	...
	function historyRow(run) {
		...
		// 复用 stepSnapshot 逻辑：历史行落盘每个节点的 id/label/type/status/output/error，
		// 使 /status 的 history 分支、bishu_workflow_status/result 的兜底查询都能展示失败原因。
		if (run.steps instanceof Map) {
			row.steps = [...run.steps.values()].map((s) => ({
				id: s.id,
				label: s.label,
				type: s.type,
				status: s.status,
		...
		} else if (Array.isArray(run.steps)) {
			row.steps = run.steps.map((s) => ({
				id: s.id,
				label: s.label,
				type: s.type,
				status: s.status,
	...
	// ── LLM calls ───────────────────────────────────────────────────
	// 模型缺省优先级：run.modelOverride / nodeModels（调用处已单独处理，不经过这里）
	//   → run.sessionModel（发起会话当前所选模型，Agent 会话级 options，修复 C）
	//   → 全局 agentDefaultModel（宿主全局默认选择）
	//   → 兜底常量
	function defaultModel(run) {
		try {
			if (run && run.sessionModel) {
				const sm = run.sessionModel;
				if (sm && sm.provider && sm.model) return { provider: sm.provider, model: sm.model };
			}
	...
	// 流式输出被掐断（内容安全拦截 / 中止）时，把已累积的部分输出存档到
	// <工作区>/cache/_partial/<run_id>_<节点id或时间戳>.partial.md，让用户能看到
	// 模型在被拦截前写了什么。拿不到工作区/run、落盘失败、路径拼接出错都静默跳过——
	// 存档失败绝不能改变原有的抛错行为（整体 try/catch，返回 null）。
	function archivePartialOutput(run, partialText, nodeId) {
		try {
			if (!run || !run.workspace || !partialText || !String(partialText).trim()) return null;
			const relDir = "cache/_partial";
			const stamp = new Date().toISOString().replace(/[:.]/g, "-").replace("T", "_").slice(0, 19);
			const tag = nodeId ? String(nodeId).replace(/[^\w-]+/g, "_") : stamp;
			const rel = relDir + "/" + run.runId + "_" + tag + ".partial.md";
			const abs = joinPath(run.workspace, rel);
			writeFile(abs, partialText);
			return rel;
		} catch (e) {
			return null;
		}
	...
	async function streamLlm(system, userText, temperature, override, run, nodeId) {
		...
		for await (const chunk of llm.stream(options)) {
			if (chunk.type === "text-delta") out += chunk.text;
				...
				if (chunk.reason.kind === "error" || chunk.reason.kind === "aborted") {
					// 掐断分支：把已累积的部分输出落盘（失败静默，不影响抛错）。
					const rel = archivePartialOutput(run, out, nodeId);
					const baseMsg = "LLM 调用失败(" + chunk.reason.kind + "): " +
						((chunk.reason.failure && chunk.reason.failure.message) || "未知错误");
					throw new Error(rel ? baseMsg + "（部分输出已存档: " + rel + "）" : baseMsg);
				}
				break;
			}
		}
	...
	async function callLlmNode(agentType, system, firstMessage, outputPath, override, run, nodeId) {
		const temperature = await agentModelParam(agentType, "temperature");
		let text = await streamLlm(system, firstMessage, temperature, override, run, nodeId);
		...
		if (isJson) {
			...
			if (!parsed.ok) {
				text = await streamLlm(system,
					...
					"\n请只返回修复后的完整 JSON，不得输出 Markdown、解释或额外文字。", temperature, override, run, nodeId);
	...
	async function runWorkflow(run) {
		...
		// 修复 D：比对 beliefs 产出的角色名集合与 skeleton 角色名集合，返回缺漏的角色名数组。
		// 仅对 character 工作流的 agent_belief 节点使用（调用处按 agent_type 特判），不做通用框架。
		// 优先解析本轮内存产出（beliefsText），磁盘文件仅作兜底（避免误读上一轮遗留数据）。
		function beliefMissingNames(run, beliefsText) {
			try {
				const skeletonPath = joinPath(run.workspace, "cache/character/skeleton.json");
				if (!fileExists(skeletonPath)) return [];
				const skeletonNames = [];
				const sk = JSON.parse(readText(skeletonPath));
				for (const c of (sk && sk.characters) || []) {
					if (c && c.name) skeletonNames.push(String(c.name));
				}
				const beliefNames = [];
				const parsed = tryParseJson(beliefsText);
				if (parsed.ok) {
					for (const b of (parsed.value && parsed.value.beliefs) || []) {
						if (b && b.character) beliefNames.push(String(b.character));
					}
				} else {
					const beliefsPath = joinPath(run.workspace, "cache/character/beliefs.json");
					if (fileExists(beliefsPath)) {
						try {
							const bd = JSON.parse(readText(beliefsPath));
							for (const b of (bd && bd.beliefs) || []) {
								if (b && b.character) beliefNames.push(String(b.character));
							}
						} catch (e) { /* ignore */ }
					}
				}
				return skeletonNames.filter((n) => !beliefNames.includes(n));
			} catch (e) {
				// 校验失败不阻断流程（保守兜底：当作无缺漏）
				return [];
			}
		}
			...
			try {
				if (node.node_type === "agent") {
					...
					while (true) {
						...
						const outputText = await callLlmNode(agentType, system, firstMessage, outputPath, nodeModel, run, node.id);
						// 修复 D：character 工作流的 agent_belief 节点（novel-character-belief）产出后
						// 校验 beliefs 角色名集合与 skeleton 角色名集合是否一致；缺漏时自动重试一次。
						if (agentType === "novel-character-belief" && outputPath && /beliefs\.json$/i.test(outputPath)) {
							const missing = beliefMissingNames(run, outputText);
							if (missing && missing.length > 0 && attempt < 2) {
								feedback = "你生成的核心信念缺少以下角色：" + missing.join("、") +
									"。请严格按照骨架中列出的每个角色逐一补齐这些角色的核心信念，不得漏掉任何一个角色。";
								st.status = "running";
								st.error = null;
								continue;
							}
						}
						// character-add 工作流的 agent_belief_add / agent_voice_add：对照点名 character_names
						// 变量校验 agent 产出，缺漏时自动重试一次；重试后仍缺则在 run.warnings 记录。
						if ((agentType === "novel-character-belief-add" || agentType === "novel-character-voice-add") &&
							outputPath && /(beliefs|voice)\.json$/i.test(outputPath)) {
							const expected = parseNamesList(run.values.character_names);
							const arrayKey = agentType === "novel-character-belief-add" ? "beliefs" : "characters";
							const nameKey = agentType === "novel-character-belief-add" ? "character" : "name";
							const label = agentType === "novel-character-belief-add" ? "核心信念" : "声线";
							const parsed2 = safeJsonParse(outputText);
							const present = new Set();
							if (parsed2 && Array.isArray(parsed2[arrayKey])) {
								for (const it of parsed2[arrayKey]) {
									const n = it && it[nameKey] ? String(it[nameKey]) : "";
									if (n) present.add(n);
								}
							}
							const missing2 = expected.filter((n) => !present.has(n));
							if (missing2.length > 0 && attempt < 2) {
								feedback = "你生成的" + label + "缺少以下点名角色：" + missing2.join("、") +
									"。请严格按照【点名角色】列表逐一为每个角色补齐" + label + "，不得漏掉任何一个角色。";
								st.status = "running";
								st.error = null;
								continue;
							}
							if (missing2.length > 0) {
								...
								run.warnings.push(agentType + " 重试后仍缺漏角色：" + missing2.join("、"));
							}
						}
						...
						// 重试后仍缺漏：不中断流程，往 run 记 WARNING（渲染层 stderr 告警兜底）。
						if (agentType === "novel-character-belief" && outputPath && /beliefs\.json$/i.test(outputPath)) {
							const stillMissing = beliefMissingNames(run, outputText);
							if (stillMissing && stillMissing.length > 0) {
								run.warnings = run.warnings || [];
								run.warnings.push("角色「" + stillMissing.join("、") + "」缺少信念数据（重试后仍缺漏），渲染层已告警留空");
							}
						}
						// B/C 后处理钩子：maintainer 节点产出后追加 meta/character_light_cards.md；
						// 失败仅 push warning，绝不阻断流程。
						if (agentType === "novel-character-maintainer") {
							try {
								const r = appendLightCards(run.workspace, outputText, run);
								if (r && r.error) {
									run.warnings = run.warnings || [];
									run.warnings.push("轻卡追加失败：" + r.error);
								} else if (r && r.appended && r.appended.length > 0) {
									run.warnings = run.warnings || [];
									run.warnings.push("轻卡追加新角色：" + r.appended.join("、"));
								}
							} catch (e) {
								run.warnings = run.warnings || [];
								run.warnings.push("轻卡追加异常：" + String((e && e.message) || e));
							}
						}
						// C 后处理钩子：character 工作流的 skeleton 节点产出后，
						// 让 agent 给一份 canon_deviations 初版（如果有产出 canon_deviations 字段就追加），
						// 没有产出也不强求——后续 post-hoc / 用户编辑都能补。
						if (agentType === "novel-character-skeleton" && outputPath && /skeleton\.json$/i.test(outputPath)) {
							try {
								const parsed = safeJsonParse(outputText);
								const initialDeviations = parsed && Array.isArray(parsed.canon_deviations)
									? parsed.canon_deviations
									: (parsed && typeof parsed.canon_deviations === "string" && parsed.canon_deviations.trim() ? parsed.canon_deviations.trim() : "");
								let text = "";
								if (typeof initialDeviations === "string") text = initialDeviations;
								else if (Array.isArray(initialDeviations)) text = initialDeviations.map((x) => "- " + String(x)).join("\n");
								if (text) {
									const r = appendCanonDeviations(run.workspace, text, null, "character", run);
									if (r && r.error) {
										run.warnings = run.warnings || [];
										run.warnings.push("canon_deviations 初版写入失败：" + r.error);
									}
								}
							} catch (e) {
								run.warnings = run.warnings || [];
								run.warnings.push("canon_deviations 初版写入异常：" + String((e && e.message) || e));
							}
						}
						// C 后处理钩子：post-hoc 章节观察员产出后，若产出 canon_deviations 字段则追加。
						if (agentType === "novel-chapter-observer" && outputPath && /obs_output\.json$/i.test(outputPath)) {
							try {
								const parsed = safeJsonParse(outputText);
								const list = parsed && Array.isArray(parsed.canon_deviations)
									? parsed.canon_deviations
									: (parsed && Array.isArray(parsed.world_diff && parsed.world_diff.canon_deviations) ? parsed.world_diff.canon_deviations : null);
								if (list && list.length > 0) {
									const text = list.map((x) => "- " + String(x)).join("\n");
									const chapter = String(run.values.chapter_number || "").trim() || null;
									const r = appendCanonDeviations(run.workspace, text, chapter, "post-hoc", run);
									if (r && r.error) {
										run.warnings = run.warnings || [];
										run.warnings.push("canon_deviations 追加失败：" + r.error);
									}
								}
							} catch (e) {
								run.warnings = run.warnings || [];
								run.warnings.push("canon_deviations 追加异常：" + String((e && e.message) || e));
							}
						}
				...
				// 把失败节点的 label 拼进 run.error，让历史行 / UI / 主动回报都能看到失败发生在哪个节点。
				const nodeLabel = node.label || node.id;
				const rawMsg = String((e && e.message) || e);
				run.error = rawMsg.startsWith("节点「")
					? rawMsg
					: "节点「" + nodeLabel + "」失败：" + rawMsg;
				const wrapped = new Error(run.error);
		...
		// character-add 工作流最终校验：核对点名角色是否都已有信念+声线，
		// 仍缺的角色 push 到 run.warnings（不动 run.status；用户决策）。
		if (run.workflowId === "character-add") {
			try {
				const expected = parseNamesList(run.values.character_names);
				if (expected.length > 0) {
					const miss = missingNamesInAdd(run, expected);
					if (miss && !miss.error) {
						if (miss.missing_belief && miss.missing_belief.length > 0) {
							run.warnings = run.warnings || [];
							run.warnings.push("character-add 最终校验：以下角色缺信念：" + miss.missing_belief.join("、"));
						}
						if (miss.missing_voice && miss.missing_voice.length > 0) {
							run.warnings = run.warnings || [];
							run.warnings.push("character-add 最终校验：以下角色缺声线：" + miss.missing_voice.join("、"));
						}
					}
				}
			} catch (e) {
				run.warnings = run.warnings || [];
				run.warnings.push("character-add 最终校验异常：" + String((e && e.message) || e));
			}
		}
	...
	// ── 后处理钩子：节点产出后追加/合并到工作区长期文件 ─────────────
	// 轻卡（meta/character_light_cards.md）：character-maintainer 节点产出后扫描新登场角色，
	// 对骨架里存在但轻卡里没有的角色追加「轻卡」条目；已有同名条目保留（用户可能手改）。
	// canon_deviations（meta/canon_deviations.md）：character 骨架环节和 post-hoc 章节后验
	// 把本章发现的「世界线/时间线/大事件」偏离追加到一份全书级清单（append-only）。
	// 两条都允许产物文件不存在（首次跑会顺手创建）。
	// 失败绝不改变 run 状态——整体 try/catch，只 push warning。

	// 解析 JSON 文本为对象，解析失败返回 null。
	function safeJsonParse(text) {
		if (text == null) return null;
		try { return JSON.parse(text); } catch (e) {
			try {
				// 兜底：去掉 markdown 围栏 + 尾部逗号 + 智能引号后再试
				let s = String(text).replace(/^```(?:json|JSON)?\s*/i, "").replace(/\s*```\s*$/, "");
	...
	// 读取工作区 JSON 文件为对象；不存在/解析失败返回 null。
	...
	// 从文本里提取「## 标题」开头的 section（用于合并模式 MD 解析）。
	...
	// 追加轻卡（meta/character_light_cards.md）：扫描骨架里所有角色，逐个核对：
	// - 如果轻卡里没有同名角色 → 追加一行「## 角色名」section（轻卡字段）
	// - 如果已有同名 section → 保留原内容（用户可能手改）
	// 返回 { appended: [...names], kept: [...names], path: rel }。
	function appendLightCards(workspace, agentText, run) {
		try {
			const cacheDir = joinPath(workspace, "cache/character");
			const skeleton = loadJsonFile(joinPath(cacheDir, "skeleton.json"));
			if (!skeleton || !Array.isArray(skeleton.characters)) {
				return { appended: [], kept: [], skipped: [], reason: "skeleton 缺失或为空" };
			}
			// 先尝试从 agent 产出文本里抽 light_cards 数组（如果新提示词给出了的话）；
			// 没抽出就退化为仅写占位条目（声线/底线/关系 = 待补充），
			// 保证字段结构稳定，用户可后续 bishu_edit_artifact 手动补全。
			let lightCardsFromAgent = [];
			const parsed = safeJsonParse(agentText);
			if (parsed && Array.isArray(parsed.light_cards)) lightCardsFromAgent = parsed.light_cards;
			else if (parsed && Array.isArray(parsed.character_light_cards)) lightCardsFromAgent = parsed.character_light_cards;
			const agentByName = new Map();
			for (const lc of lightCardsFromAgent) {
				if (lc && lc.character_name) agentByName.set(String(lc.character_name), lc);
			}

			const metaDir = joinPath(workspace, "meta");
			const relMd = "meta/character_light_cards.md";
			const absMd = joinPath(workspace, relMd);
			mkdirSync(metaDir, { recursive: true });
			const existing = fileExists(absMd) ? readText(absMd) : "";
			const existingTitles = new Set(parseMdSections(existing).map((s) => s.title));
			// 「## 角色名（别名、别名2）」的标题拆出去核别名集合，兼容轻卡标题里带别名的情况
			...
			let nextMd = existing;
			if (nextMd && !nextMd.endsWith("\n")) nextMd += "\n";
			for (const ch of skeleton.characters) {
				const name = ch && ch.name ? String(ch.name) : "";
				if (!name) continue;
				if (existingTitles.has(name) || existingNames.has(name)) {
					kept.push(name);
					continue;
				}
				const lc = agentByName.get(name) || {};
				const voiceLine = lc.voice_line ? String(lc.voice_line).trim() : "（待补充）";
				const bottomLine = lc.bottom_line ? String(lc.bottom_line).trim() : "（待补充）";
				const relation = lc.relation_to_main ? String(lc.relation_to_main).trim() : "（待补充）";
				const deviation = lc.canon_deviation ? String(lc.canon_deviation).trim() : "无";
				const firstChapter = lc.first_chapter ? String(lc.first_chapter).trim() : "";
				const header = `## ${name}\n`;
				const block =
					header +
					`- 声线：${voiceLine}\n` +
			...
			if (appended.length > 0) {
				writeFile(absMd, nextMd);
				run.produced = run.produced || [];
				if (!run.produced.includes(relMd)) run.produced.push(relMd);
			}
			return { appended, kept, path: relMd };
		} catch (e) {
			return { appended: [], kept: [], error: String((e && e.message) || e) };
		}
	}

	// 追加 canon_deviations（meta/canon_deviations.md）：append-only。
	// body：自由文本（多行 Markdown 也行），空文本不写文件。
	// chapter：可选章节号；空时不带章节标签。
	// source：来源标签（"character"|"post-hoc"），便于人读区分。
	function appendCanonDeviations(workspace, body, chapter, source, run) {
		try {
			const text = String(body || "").trim();
			if (!text) return { appended: false, reason: "无内容" };
			const rel = "meta/canon_deviations.md";
			const abs = joinPath(workspace, rel);
			mkdirSync(dirname(abs), { recursive: true });
			const stamp = new Date().toISOString().slice(0, 10);
			const header = chapter
				? `## 章节 ${chapter}（${source}）· ${stamp}\n\n`
				: `## 初版（${source}）· ${stamp}\n\n`;
			const block = header + text.replace(/\n+$/, "") + "\n\n---\n\n";
			const prev = fileExists(abs) ? readText(abs) : "";
			let next;
			if (prev && !prev.endsWith("\n")) next = prev + "\n" + block;
			else next = prev + block;
			writeFile(abs, next);
			run.produced = run.produced || [];
			if (!run.produced.includes(rel)) run.produced.push(rel);
			return { appended: true, path: rel };
		} catch (e) {
			return { appended: false, error: String((e && e.message) || e) };
		}
	}

	// 合并 JSON 追加：把 agentText 解析出的对象数组，append 到 workspace 下
	// fileRel 指向的 JSON 文件里（追加对象按 nameKey 去重，已存在的跳过）。
	// 追加完写回原文件。失败返回错误对象，绝不抛错。
	// 返回 { added: [...names], skipped: [...names], total: number, path: rel }。
	function mergeJsonAppend(workspace, agentText, fileRel, arrayKey, nameKey) {
		try {
			const parsed = safeJsonParse(agentText);
			if (!parsed || !Array.isArray(parsed[arrayKey])) {
				return { added: [], skipped: [], total: 0, error: "agent 产出不含 " + arrayKey + " 数组" };
			}
			const abs = joinPath(workspace, fileRel);
			mkdirSync(dirname(abs), { recursive: true });
			let existing = { [arrayKey]: [] };
			if (fileExists(abs)) {
				const cur = safeJsonParse(readText(abs));
				if (cur && Array.isArray(cur[arrayKey])) existing = cur;
			}
			const existingNames = new Set();
			for (const e of existing[arrayKey]) {
				const n = e && e[nameKey] ? String(e[nameKey]) : "";
				if (n) existingNames.add(n);
			}
			const added = [];
			const skipped = [];
			for (const it of parsed[arrayKey]) {
				const n = it && it[nameKey] ? String(it[nameKey]) : "";
				if (!n) { skipped.push("(无 " + nameKey + ")"); continue; }
				if (existingNames.has(n)) { skipped.push(n); continue; }
				existing[arrayKey].push(it);
				existingNames.add(n);
				added.push(n);
			}
			if (added.length > 0) writeFile(abs, JSON.stringify(existing, null, 2));
			return { added, skipped, total: existing[arrayKey].length, path: fileRel };
		} catch (e) {
			return { added: [], skipped: [], total: 0, error: String((e && e.message) || e) };
		}
	}

	// 校验：对照 expectedNames 列表，检查 beliefs.json/voice.json 里是否齐全。
	// 名字按 nameKey（belief 是 "character"，voice 是 "name"）匹配。
	// 返回 { missing_belief: [...], missing_voice: [...] }，空数组表示齐全。
	function missingNamesInAdd(run, expectedNames) {
		try {
			const cacheDir = joinPath(run.workspace, "cache/character");
			const beliefsPath = joinPath(cacheDir, "beliefs.json");
			const voicePath = joinPath(cacheDir, "voice.json");
			const beliefNames = new Set();
			const beliefs = loadJsonFile(beliefsPath);
			if (beliefs && Array.isArray(beliefs.beliefs)) {
				for (const b of beliefs.beliefs) {
					if (b && b.character) beliefNames.add(String(b.character));
				}
			}
			const voiceNames = new Set();
			const voice = loadJsonFile(voicePath);
			if (voice && Array.isArray(voice.characters)) {
				for (const c of voice.characters) {
					if (c && c.name) voiceNames.add(String(c.name));
				}
			}
			const missing_belief = expectedNames.filter((n) => !beliefNames.has(n));
			const missing_voice = expectedNames.filter((n) => !voiceNames.has(n));
			return { missing_belief, missing_voice };
		} catch (e) {
			return { missing_belief: [], missing_voice: [], error: String((e && e.message) || e) };
		}
	}

	// 把 character_names 字符串/JSON 数组归一为 string[]。支持三种入参：
	// - JSON 数组字符串 '["甲","乙"]'
	// - 逗号分隔 '甲,乙,丙'（中英文逗号都吃）
	// - 单个名字 '甲'
	// 空/解析失败返回 []。
	function parseNamesList(raw) {
		...
		try {
			...
			if (typeof j === "string") return [j.trim()].filter(Boolean);
		} catch (e) { /* fall through */ }
		// 逗号分隔（中英文逗号都吃）
	...
	// 终态主动回报：工作流完成后向发起会话注入一条 user 消息并唤醒 agent 开新一轮。
	// 消息 source 标注插件来源（{ kind: "plugin", plugin: "dsh-bishu-novel", form: "notice" }），
	// 与宿主 dsh-goal-round-driver / dsh-schedule 的 followup 用法一致（见宿主
	// dsh-llm/lib/types/message.d.ts 的 MessageSourceMap + ContextFormed）。
	// 通知失败绝不能影响工作流状态落盘：整体包 try/catch，ownerAgent 缺失时静默跳过。
	function notifyRunFinished(run) {
		try {
			const agent = run.ownerAgent;
			if (!agent || typeof agent.followup !== "function") return;
			let wfName = run.workflowId;
			try { wfName = workflow(run.workflowId).name || run.workflowId; } catch (e) { /* keep id */ }
			const terminal = run.status === "completed" || run.status === "failed";
			if (!terminal) return;
			let text = "【Bishu Novel 工作流结束 — 需要你行动】\n\n工作流：" + wfName +
				"\nrun_id：" + run.runId +
				"\n状态：" + (run.status === "completed" ? "completed（成功）" : "failed（失败）");
			if (run.status === "failed") {
				text += "\n\n失败原因：\n" + (run.error || "未知错误") +
					"\n\n请向用户如实汇报失败原因（错误原文见上），不要再重试前等待通知。";
			} else {
				const files = run.produced || [];
				if (files.length > 0) {
					text += "\n\n产物文件：\n" + files.map((f) => "- " + f).join("\n");
				} else {
					text += "\n\n本次运行未产生新文件。";
				}
				text += "\n\n请立即调用 bishu_workflow_result（run_id=" + run.runId +
					"）核验产出，检查工作区落盘文件，然后向用户汇报结果。工作流已经结束，不要再等待，也不要再声称它仍在运行。";
			}
			agent.followup({
				content: [{ type: "text", text }],
				source: { kind: "plugin", plugin: "dsh-bishu-novel", form: "notice", summary: "Bishu Novel 工作流 " + wfName + " " + run.status + "（" + run.runId + "）" },
			});
		} catch (e) {
			console.error("[dsh-bishu-novel] 工作流终态通知失败（不影响运行状态）:", e && e.message);
		}
	}

	function startRun(workflowId, workspace, parameters, modelOverride, nodeModels, approval, ownerAgent, sessionModel) {
		...
		const run = {
			...
			// 发起会话的 Agent（工具 execute 的第二参数 exec.agent），终态后用于 followup 主动回报；
			// GUI 发起（HTTP API）时不存在，静默跳过。
			ownerAgent: ownerAgent || null,
			// 发起会话当前所选模型（Agent 会话级 options.provider/model），作为缺省模型继承的中间层。
			sessionModel: sessionModel && sessionModel.provider && sessionModel.model
				? { provider: String(sessionModel.provider), model: String(sessionModel.model) }
				: null,
	...
	tool("bishu_run_workflow",
		"在指定书籍工作区启动一条 Bishu Novel 工作流，返回 run_id 后异步执行。parameters 里按变量 key 覆盖（如 premise、genre、language、chapter_number、prev_chapter、human_intent、writer_type 等）。model 可选：指定本次运行的整体模型；node_models 可选：按 agent 节点（agent_type）单独指定模型，优先级高于 model；两者都不传时默认使用发起会话当前所选模型（会话级模型选择）。用 bishu_list_models 查看可用项。同一本书的所有工作流必须用同一个 workspace 目录。",
		{
			...
			model: { type: "object", additionalProperties: true, description: "可选：运行级模型覆盖 {\"provider\":\"...\",\"model\":\"...\"}；不传时默认使用发起会话当前所选模型" },
		...
		async (args, exec) => {
			// exec.agent 是发起调用的会话 Agent（dsh-tools ToolRunContext 字段，见宿主
			// dsh-tools/lib/types/index.d.ts ToolRunContext extends ToolExecution.agent）。
			// 终态后用它 followup 主动回报；其 options 提供会话级 provider/model 作为缺省模型继承。
			const ownerAgent = exec && exec.agent;
			const sessionModel = ownerAgent && ownerAgent.options && ownerAgent.options.provider && ownerAgent.options.model
				? { provider: ownerAgent.options.provider, model: ownerAgent.options.model }
				: null;
			const run = startRun(String(args.workflow_id), String(args.workspace), args.parameters || {}, args.model, args.node_models, args.approval, ownerAgent, sessionModel);
			return {
				run_id: run.runId,
				workflow_id: run.workflowId,
				workspace: run.workspace,
	...
	tool("bishu_workflow_status",
		...
		async (args) => {
			...
			if (!run) {
				// 进程重启后内存 Map 已清空：从持久化历史按 run_id 兜底查询，
				// 历史行含 status/error/steps，仍能给出失败原因，不再直接报「未找到运行」。
				const row = loadHistory().find((e) => e.run_id === String(args.run_id));
				if (!row) return { error: "未找到运行 " + args.run_id };
				return {
					run_id: row.run_id, workflow_id: row.workflow_id, workflow_name: row.workflow_name,
					status: row.status, current_node: null, steps: row.steps || [], error: row.error,
					produced_files: row.produced_files || [], elapsed_ms: row.elapsed_ms,
					note: "该运行来自持久化历史（当前进程未持有内存态）",
				};
			}
			return {
	...
	tool("bishu_workflow_result",
		...
		async (args) => {
			const run = runs.get(String(args.run_id));
			if (!run) {
				// 兜底：从持久化历史按 run_id 读取，历史行提供 status/error/steps/elapsed 等可得字段。
				const row = loadHistory().find((e) => e.run_id === String(args.run_id));
				if (!row) return { error: "未找到运行 " + args.run_id };
				return {
					run_id: row.run_id, workflow_id: row.workflow_id, workflow_name: row.workflow_name,
					...
					produced_files: row.produced_files || [],
					steps: row.steps || [],
					note: "该运行来自持久化历史（当前进程未持有内存态）",
				};
			}
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					id = String(queryParam(url, "workflow_id") ?? "");
					const wf = workflow(id);
					writeJson(res, 200, { workflow: workflowSummary(wf), model_prefs: getPrefs(id) });
				} catch (e) {
					writeJson(res, 400, { error: "未知工作流: " + (id || "(未提供 workflow_id)") });
				}
			},
		},
		{
				...
				try {
					...
					if (!run) {
						...
						writeJson(res, 200, {
							...
							current_node: null, steps: row.steps || [], error: row.error,
			...
			handler: async (req, res) => {
				try {
					...
					if (!run) {
						...
						writeJson(res, 200, {
							run_id: row.run_id, workflow_id: row.workflow_id, status: row.status, error: row.error,
							elapsed_ms: row.elapsed_ms,
							produced_files: row.produced_files || [],
							steps: row.steps || [],
